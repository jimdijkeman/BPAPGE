...
def get_isoforms_from_fasta(fasta_file: str):

    data_dict = {}

    for record in SeqIO.parse(fasta_file, "fasta"):
        if "isoform" in record.description:
            description_without_prefix = record.description.split("ALLSI ")[1]
            name_end = description_without_prefix.index("OS=")
            long_name = description_without_prefix[:name_end].strip()

            sequence = str(record.seq)

            for item in record.description.split():
                if item.startswith("GN="):
                    gn_value = item.split("=")[1]
                    data_dict[gn_value] = {'name': long_name, 'sequence': sequence}
                    break
